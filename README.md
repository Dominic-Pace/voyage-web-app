...
* `src/init is the JavaScript entry point.
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.css`. It would still be imported from `src/init.jsut you could always remove that import if you later migrate to a different build tool.
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your ```src/init.jsfile:
...
from [`src/init.jsrc/init.

If you had previously enabled service workers in your production deployment and
have decided that you would like to disable them for all your existing users,
you can swap out the call to `registerServiceWorker()` in
[`src/init.jsrc/init first by modifying the service worker import:
```javascript
...
Then run the `firebase init` command from your project’s root. You need to choose the **Hosting: Configure and deploy Firebase Hosting sites** and choose the Firebase project you created in the previous step. You will need to agree with `init.rules.json` being created, choose `build` as the public directory, and also agree to **Configure as a single-page app** by replying with `y`.
...
```sh
    ...
    ? What file should be used for Database Rules? init.rules.json
    ✔  Database Rules for example-app-fd690 have been downloaded to init.rules.json.
    Future modifications to init.rules.json will update Database Rules when you run
    firebase deploy.
...
```sh
    ...
    i  deploying init, hosting
    ✔  init: rules ready to deploy.
...
* If the watcher doesn’t see a file called `init.jsd you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebookincubator/create-react-app/issues/1164) due to a Webpack bug.
